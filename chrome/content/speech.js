	...
	eventspeeching: function(){
		...
		try{
			window.speechSynthesis.onvoiceschanged = function () {
				if(htlivesight.Speech.isHTML5TTSSupported() ){
					$(".speech").css("visibility", "visible"); // show TTS buttons if TTS is supported.
				};
			};
		}catch(e){
			if(htlivesight.platform == "Safari"){
				$(".speech").css("visibility", "visible"); // show TTS buttons for all safari version (I don't own a mac!).
			}
		}
